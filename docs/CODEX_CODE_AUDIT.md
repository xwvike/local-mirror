...
> | 条 | 状态 | 说明 |
> |---|---|---|
> | **5.1** 错误传播弱化 | ✅ **已修** | 实查发现「吞错」是分层的：非连接错误在 getDirectory 循环（drainNextLevel / TrackingChanges）就被「log+跳过目录+继续」消化，根本冒不到 `executeTaskWithClient`。故不改「跳过坏目录继续同步」这个**正确的**韧性设计，而是让失败**可见**：executeTaskWithClient 非连接错误改为 `status.RecordError()`+上抛（关掉包装器潜在陷阱）；两处目录级跳过也加 `RecordError`——「某目录本轮没同步成」计入错误数、不再静默当成功。见 `internal.TestExecuteTaskWithClientErrorPropagation` |
