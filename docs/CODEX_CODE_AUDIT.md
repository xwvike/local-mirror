...
> 未逐条交代。现补齐。**其中 5.3 / 5.4 / 5.7 已修；5.1/5.2/5.5/5.6 未做，
...
- **§5 工程债务**（逐条去向见 §5 顶部评估块）：5.3 已修（重建节流 + 漂移强制触发）、
  5.4 已修（全局文件服务信号量）、5.7 已处理（docs ADR 化）；**5.1（错误语义梳理）/
  5.5（去全局化/库化）/ 5.6（拆大文件）登记为待决重构**，动手前需单独立项。5.2（多次哈希）为设计权衡。
