...
> 未逐条交代。现补齐。**其中 5.3 已修、5.7 已处理、5.4 被 SEC-01 部分缓解；5.1/5.2/5.5/5.6 未做，
> 且 5.2/5.5/5.6 被本轮外科手术式的补丁轻微加重**——如实记录：
>
> | 条 | 状态 | 说明 |
> |---|---|---|
> | **5.1** 错误传播弱化 | ⏳ 延后 | `executeTaskWithClient` 吞非连接错误返 nil 属实。COR-01 的重建失败也沿用了「Warn 后继续」同款模式。梳理「可跳过单项错误 vs 本轮任务失败」是行为语义改动，需单独评估 |
> | **5.2** 多次完整哈希 | ⏳ 延后·略加重 | 完整性+续传的设计后果。COR-02 的 rename 前重验、COR-01 的重建都**新增了哈希**（为正确性，故意），这条债往上加了一点 |
...
> | **5.4** 缺全局 I/O 限流 | 🟡 部分缓解 | SEC-01 逼监听端必须带密钥，「未认证监听被当磁盘/CPU 放大器」的最坏面被堵；但「256 连接各自触发大文件预哈希」的全局限流未加，延后 |
...
- **§5 工程债务**（逐条去向见 §5 顶部评估块）：5.3 已修（重建节流 + 漂移强制触发）、5.7 已处理
  （docs ADR 化）、5.4 被 SEC-01 部分缓解；**5.1（错误语义梳理）/ 5.5（去全局化/库化）/
  5.6（拆大文件）登记为待决重构**，动手前需单独立项。5.2（多次哈希）为设计权衡。
